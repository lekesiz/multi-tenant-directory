...
/**
 * Hierarchical Category Seed with Google Places Types Mapping
 * This seed creates a professional category hierarchy with parent-child relationships
 * and maps Google Places API types to our categories
 */
...
async function main() {
  console.log('🌱 Starting category hierarchy seed...');
...
- Child categories: ${childCategories}
  ...
  allCategories.forEach((parent) => {
    console.log(`\n${parent.icon} ${parent.nameFr} (${parent.googleTypes.length} Google types)`);
    parent.children.forEach((child) => {
      console.log(`  ├─ ${child.icon} ${child.nameFr} (${child.googleTypes.length} Google types)`);
    });
  });
}
