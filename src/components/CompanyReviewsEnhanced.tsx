...
import { ReviewVerifiedBadge } from './ReviewVerifiedBadge';
...
interface CompanyReviewsEnhancedProps {
  companyId: number;
  companyName: string;
  isBusinessOwner?: boolean;
}

type SortOption = 'recent' | 'oldest' | 'highest' | 'lowest' | 'helpful';
type FilterOption = 'all' | 'google' | 'manual' | 'verified' | 'with-replies';

export default function CompanyReviewsEnhanced({ 
  companyId, 
  companyName,
  isBusinessOwner = false 
}: CompanyReviewsEnhancedProps) {
  ...
  const stats = useMemo(() => {
    ...
    const withRepliesCount = reviews.filter(r => r.reply).length;

    return { total, avgRating, distribution, verifiedCount, withRepliesCount };
  }, [reviews]);
  ...
  const filteredAndSortedReviews = useMemo(() => {
    ...
    switch (filterBy) {
      case 'google':
        filtered = reviews.filter((r) => r.source === 'google');
        break;
      case 'manual':
        filtered = reviews.filter((r) => r.source === 'manual');
        break;
      case 'verified':
        filtered = reviews.filter((r) => r.isVerified);
        break;
      case 'with-replies':
        filtered = reviews.filter((r) => r.reply);
        break;
    }
  ...
  const handleVerifyReview = async (reviewId: number) => {
    try {
      const response = await fetch(`/api/business/reviews/${reviewId}/verify`, {
        method: 'PATCH',
      });
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {stats.verifiedCount > 0 && (
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">
                        {stats.verifiedCount} vérifié{stats.verifiedCount > 1 ? 's' : ''}
                      </span>
                    )}
                    {stats.withRepliesCount > 0 && (
                      <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                        {stats.withRepliesCount} avec réponse
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className="flex-1">
              ...
              >
                ...
                <option value="verified">
                  Vérifiés ({reviews.filter((r) => r.isVerified).length})
                </option>
                <option value="with-replies">
                  Avec réponse ({reviews.filter((r) => r.reply).length})
                </option>
              </select>
            </div>
                    ...
                    <div className="flex items-start space-x-3 sm:space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <ReviewVerifiedBadge isVerified={review.isVerified || false} />
                            </div>
                          ...
                          {isBusinessOwner && !review.isVerified && (
                            <button
                        ...
                        <div className="flex items-center justify-between mt-4">
                          ...
                          />
                          {!isBusinessOwner && <ReviewReportModal reviewId={review.id} />}
                        </div>
                      </div>
                    </div>
                  </div>
